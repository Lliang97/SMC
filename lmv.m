...
parfor ji=1:num
    ff=-2*B(:,ji)'*H{j}';
    %Z(:,ji)=quadprog(A,ff',[],[],ones(1,r),1,zeros(r,1),ones(r,1),[],options);%original
    Z(:,ji)=quadprog(A,ff',[],[],[],[],zeros(r,1),ones(r,1),[],options);% no sum limit
    %Z(:,ji)=quadprog(A,ff',[],[],ones(1,r),0.8,zeros(r,1),ones(r,1),[],options);% constraint limit
end
    S{j}=Z;
    Sbar=cat(1,Sbar,S{j});
